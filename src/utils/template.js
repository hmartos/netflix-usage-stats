...
function fillDashboardTemplate() {
  ...
  document.querySelector('#moviesVsTvTime .ns-title').textContent = chrome.i18n.getMessage('moviesVsTvTime');
  document
    .querySelector('#moviesVsTvTime .ns-title-container')
    .setAttribute('aria-label', chrome.i18n.getMessage('moviesVsTvTime'));

  document.querySelector('#meanTimeByWeekDay .ns-title').textContent = chrome.i18n.getMessage('meanTimeByWeekDay');
  document
    .querySelector('#meanTimeByWeekDay .ns-title-container')
    .setAttribute('aria-label', chrome.i18n.getMessage('meanTimeByWeekDay'));

  document.querySelector('#activity .ns-title').textContent = chrome.i18n.getMessage('viewingActivity');
  document
    .querySelector('#activity .ns-title-container')
    .setAttribute('aria-label', chrome.i18n.getMessage('viewingActivity'));

  document.querySelector('#activityDataTable #dataTableTitle').textContent = chrome.i18n.getMessage('title');
  document.querySelector('#activityDataTable #dataTableDateFormatted').textContent = chrome.i18n.getMessage('date');
  document.querySelector('#activityDataTable #dataTableDurationFormatted').textContent = chrome.i18n.getMessage(
    'duration'
  );
  document.querySelector('#activityDataTable #dataTableType').textContent = chrome.i18n.getMessage('type');
}
